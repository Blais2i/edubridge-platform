  ...
  return (
    <div className="w-64 h-full bg-slate-900 text-white p-4 flex flex-col rounded-xl">
      ...
      <button
        type="button"
        onClick={onNewChat}
        className="mb-4 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-lg px-3 py-2 shrink-0 text-white font-medium hover:opacity-90 transition"
      >
        + New chat
      </button>

      <div className="text-sm text-slate-300 mb-2 shrink-0">Recent</div>
      ...
      <div className="flex-1 overflow-y-auto space-y-1 pr-1 sidebar-scrollbar">
        {conversations.map((c) => (
          ...
          >
            <button onClick={() => onSelectConversation(c.id)} className="flex-1 text-left truncate">
              {c.title || "New chat"}
            </button>

            <button
              onClick={() => setOpenChatMenuId(openChatMenuId === c.id ? null : c.id)}
            ...
            {openChatMenuId === c.id && (
              ...
              >
                <button onClick={() => renameChat(c.id, c.title)} className="w-full text-left px-3 py-2 hover:bg-white/10">
                  Rename
                </button>
                <button
      ...
      <div ref={accountMenuRef} className="mt-4 pt-4 border-t border-slate-700 relative">
        <button
        ...
        >
          ...
          <div className="text-left">
            <p className="text-sm font-medium">{user?.user_metadata?.full_name || "Student"}</p>
        ...
        {accountMenuOpen && (
          <div className="absolute bottom-14 left-0 w-full bg-slate-800 rounded-lg shadow-lg overflow-hidden text-sm">
            ...
            <button onClick={handleLogout} className="w-full text-left px-4 py-2 hover:bg-white/10 text-red-400">
              Log out
            </button>
          </div>
        )}
